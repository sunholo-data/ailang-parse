...
@pytest.mark.skipif(SKIP, reason=REASON)
class TestUnstructuredFileUploadIntegration:
    """Test Unstructured compat with local file upload (multipart)."""

    def test_partition_file_upload(self, tmp_path):
        """Create a minimal DOCX and upload via UnstructuredClient.partition()."""
        import zipfile
        docx_path = tmp_path / "unstruct_test.docx"
        with zipfile.ZipFile(docx_path, "w") as zf:
            ...
            zf.writestr("word/document.xml", '<?xml version="1.0" encoding="UTF-8"?>'
                ...
                '<w:body><w:p><w:r><w:t>Unstructured upload test</w:t></w:r></w:p></w:body>'
                '</w:document>')

        uc = UnstructuredClient()
        try:
            elements = uc.general.partition(file=str(docx_path))
        except DocParseError as e:
            pytest.skip(f"Partition not available: {e}")
        assert len(elements) > 0
        for el in elements:
